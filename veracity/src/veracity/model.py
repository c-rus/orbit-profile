...
from . import to_bin, from_bin, write_bits, pow2m1
...
class SuperBfm(_ABC):        
    ...
    def send(self, fd, mode: Mode):
        ...
        result = []
        port: Signal
        for key, port in self.get_ports(mode=mode):
...
procedure scoreboard(file fd: text) is 
    variable row : line;
    variable ideal : '''+self.entity+'''_bfm;
begin
    if endfile(fd) = false then
...
''' 
        ...
        for id, port in self.get_ports(mode=Mode.OUTPUT):
            fn_call = 'load_var' if port.is_single_ended() == False else 'load_var_single'
            result += '        '+fn_call+'(row, ideal.'+id+');\n'
            if port.is_single_ended() == True:
                result += '        assert_eq(as_logics(bfm.'+id+'), as_logics(ideal.'+id+'), \"'+id+'\");\n'
            else:
                result += '        assert_eq(bfm.'+id+', ideal.'+id+', \"'+id+'\");\n'
            pass
        result += '''    end if;
end procedure;
'''
...
type '''+self.entity+'''_bfm is record
'''
        id: str
        port: Signal
        ...
        for (id, port) in self.get_ports(mode=None):
            if port.is_single_ended() == True:
                result += '    '+id+' '+(' ' * (longest_len-len(str(id))))+': std_logic'+';\n'
            else:
                result += '    '+id+' '+(' ' * (longest_len-len(str(id))))+': std_logic_vector('
                # check the 'downto' and 'to' attributes
                if port._downto is not None:
                    result += str(port._downto[0])+' downto '+str(port._downto[1])
                elif port._to is not None:
                    result += str(port._to[0])+' to '+str(port._to[1])
                else:
                    result += str(port.width()-1)+' downto '+'0'
                result += ');\n'

            pass
        result += '''end record;

signal bfm : '''+self.entity+'''_bfm;
'''
        return result
